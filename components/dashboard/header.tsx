...
export function Header({ studentName, courseLabel, viewMode, isAdmin, onLogout, onMobileMenuToggle }: HeaderProps) {
  return (
    <header className="bg-white/95 backdrop-blur-md border-b border-slate-200 px-4 md:px-8 py-3 md:py-4">
      <div className="flex justify-between items-center gap-4">
        ...
        <div className="flex-1 min-w-0">
          <h1 className="text-base md:text-xl font-light text-slate-900 truncate">
            ...
            {courseLabel && (
              <span className="text-slate-500 font-normal"> · {courseLabel}</span>
            )}
          </h1>
        </div>
      ...
      <div className="mt-3 px-3 md:px-4 py-2 bg-slate-50 border border-slate-200 rounded-lg">
        <p className="text-xs md:text-sm text-slate-700 font-light">

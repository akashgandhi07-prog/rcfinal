...
export default function PortalPage() {
  ...
  useEffect(() => {
    if (user) {
      ...
      if (user.target_course === "veterinary") {
        setShowUCAT(false)
  ...
  const handleOnboardingComplete = async (data: OnboardingData) => {
    ...
    const updates: {
      ...
      gcse_summary: data.gcse_summary || null,
      a_level_predictions: data.a_level_predictions || null,
  ...
  // Handle parent role - show student selection if they have multiple linked students
  if (user?.role === "parent") {
    // Parents view their linked students
    // For now, just show the first linked student or allow selection
    // This should be enhanced with a student selector
  }
  ...
  return (
    <DashboardShell
      ...
      showUCAT={showUCAT}
    >
      {activeView === "dashboard" && <DashboardView viewMode={viewMode} />}
      {activeView === "profile" && <ProfileView viewMode={viewMode} userData={user} />}
      {activeView === "portfolio" && <PortfolioBuilder viewMode={viewMode} />}
      {activeView === "ucat" && showUCAT && <UCATTracker viewMode={viewMode} />}
      {activeView === "strategy" && <StrategyKanban viewMode={viewMode} userData={user} />}
